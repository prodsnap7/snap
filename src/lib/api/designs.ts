...
export const updateDesign = async ({ id, data }: { id: string; data: Partial<DesignType>}) => {
  const dto = convertToPartialCreateDesignDTO(data);
  const res = await client.put("/designs/" + id, dto);
...
function convertToPartialDesignType(dto: Partial<CreateDesignDTO>): Partial<DesignType> {
  ...
  if (dto.canvasWidth !== undefined || dto.canvasHeight !== undefined || dto.background !== undefined || dto.elements !== undefined || dto.fonts !== undefined) {
    design.canvas = {};
