...
export function snapToGridOld(
	...
	for (const otherEl of otherEls) {
		if (
			isCloseEnough(currentEl.x, otherEl.x, snapThreshold) ||
			isCloseEnough(currentEl.x + currentEl.width, otherEl.x + otherEl.width, snapThreshold)
		) {
			bestSnapX = isCloseEnough(currentEl.x, otherEl.x, snapThreshold)
				? otherEl.x
				: otherEl.x + otherEl.width - currentEl.width;
		}

		if (
			isCloseEnough(currentEl.y, otherEl.y, snapThreshold) ||
			isCloseEnough(currentEl.y + currentEl.height, otherEl.y + otherEl.height, snapThreshold)
		) {
			bestSnapY = isCloseEnough(currentEl.y, otherEl.y, snapThreshold)
				? otherEl.y
				: otherEl.y + otherEl.height - currentEl.height;
		}
